...
public class SetupEnchantsettings {
	AutoMiner m;
	File file;
	YamlConfiguration ymlconfig;

	public SetupEnchantsettings() {
		this.file = new File(AutoMiner.getInstance().getDataFolder(), "autominer_enchants.yml");
		if (!this.file.exists()) {
			AutoMiner.getInstance().saveResource("autominer_enchants.yml", false);
		}
	...
	public double getDouble(String str) {
		if (this.ymlconfig.contains(str)) {
			return this.ymlconfig.getDouble(str);
		}
		return 0.0;
	}
